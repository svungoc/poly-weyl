...
(* Algebra over a field, with unit *)
...
module AlgebraOfField (F : Field) :
  (Algebra with type t = F.t and type scalar = F.t) = struct
  include F
  type scalar = t
  let scal_mul = mul
  let of_scalar x = x
...
(* Generic Mulivariate Polynomials and Weyl Algebra *)
...
module Monomial_generic :
  Monomial with type names = (int -> string option) = struct
  type t = { map : int Imap.t; (* i --> a_i *)
             support : Iset.t;
             (* The support is not necessary as it can be retrieved from the map
                using [support_from_map]. Only for optimization (?) *)
           }
...
module Polynomial_make (M : Monomial) (R : Ring) = struct
...
module Polynomial_generic (R : Ring) = Polynomial_make (Monomial_generic) (R)
...
module Polynomial1_make (R : Ring) = struct
  include Polynomial_make (Monomial1) (R)
  module PG = Polynomial_generic (R)
  type generic = PG.t
...
module RealPoly = Polynomial_generic (RealNumbers)
module RealPoly1 = Polynomial1_make (RealNumbers)
module RatPoly = Polynomial_generic (Rationals)
module RatPoly1 = Polynomial1_make (Rationals)
